...
def get_random_dish():
    ...
    db = get_db()
    dishes = [{"id": str(dish['_id']), "name": dish['name'], "calories": dish['cal'], "ingredients": dish['ingredients'],
               "prepare_steps": dish['prepare_steps'], "cook_steps": dish['cook_steps'], "image": dish['link_img']} for dish in db['Dishes'].find()]
...
def search_in_array_and_return_dish_that_contain_ingredients():

    dishes_that_have_ingredients = []

    # get list off ingredients
    try:
        request_ingredients = request.args.get(
    ...
    dishes_in_db = [{"id": str(dish['_id']), "name": dish['name'], "calories": dish['cal'], "ingredients": dish['ingredients'],
                     "prepare_steps": dish['prepare_steps'], "cook_steps": dish['cook_steps'], "image": dish['link_img']} for dish in _dishes]
    if not dishes_in_db:
        return jsonify([]), 404
    ...
    list_of_dishes_sorted_by_score = sorted(
        dishes_that_have_ingredients, key=itemgetter('score'), reverse=True)
...
@cross_origin()
@app.route('/dish/all', methods=['GET'])
def get_all_dishes():
    db = get_db()
    _dishes = db['Dishes'].find()
    dishes = [{"id": str(dish['_id']), "name": dish['name'], "calories": dish['cal'], "ingredients": dish['ingredients'],
...
def get_dish_by_id(id):
    ...
    dish = {"id": str(dish['_id']), "name": dish['name'], "calories": dish['cal'], "ingredients": dish['ingredients'],
            "prepare_steps": dish['prepare_steps'], "cook_steps": dish['cook_steps'], "image": dish['link_img']}
    if not dish:
        return jsonify([]), 404
...
def get_dish_by_name(name):
    ...
    this_dish = [{"id": str(dish['_id']), "name": dish['name'], "calories": dish['cal'], "ingredients": dish['ingredients'],
                  "prepare_steps": dish['prepare_steps'], "cook_steps": dish['cook_steps'], "image": dish['link_img']} for dish in _dish]
    if not this_dish:
        return jsonify([]), 404
...
def get_all_ingredients():
    ...
    ingredients = [{'id': i['id'], 'name': i['name'],
                    'calories': i['cal'], 'image': i['link_img']} for i in _ingredients]
    if not ingredients:
        return jsonify([]), 404
...
def get_ingredient_by_id(id):
    ...
    ingredient = [{'id': i['id'], 'name': i['name'],
                   'calories': i['cal'], 'image': i['link_img']} for i in _ingredient]
    if not ingredient:
        return jsonify([]), 404
