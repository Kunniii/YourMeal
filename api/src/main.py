...
def search_in_array_and_return_dish_that_contain_ingredients():
    ...
    try:
        request_ingredients = (
            request.args.get("ingredients").replace(", ", ",").split(",")
        )
        ...
        return (
            jsonify(
                {
                    "message": "array name must be `ingredients` or some errors have orcured!"
                }
            ),
            400,
        )
...
def get_all_ingredients():
    ...
    ingredients = [
        {"id": i["id"], "name": i["name"], "calories": i["cal"], "image": i["link_img"]}
